...
const Navbar = ({ active }: Props) => {
   ...
   return (
      <motion.nav
         initial={{ y: -100 }}
         animate={{ y: 0 }}
         transition={{ type: 'spring', delay: 1.8, stiffness: 100 }}
         className="fixed top-0 z-50 w-screen border-b bg-white px-[5%]"
      >
         <motion.div
            variants={{
         ...
         <div className="flex h-16  w-full flex-row items-center justify-between py-2 sm:h-14">
            ...
            <ul className="relative flex h-full w-full sm:w-3/5 lg:w-5/12 ">
               ...
               <motion.div
                  ...
                  className="absolute bottom-0 left-0 h-full w-1/4 rounded-2xl bg-purple-500 shadow-lg shadow-violet-300"
               ></motion.div>
            </ul>
         </div>
      </motion.nav>
